...
def update_inventory_chart(n_clicks, adjust_clicks, store, product, apply_weather, location, stock_adjustment, adjustment_date=None):
    try:
        ...
        if 'Stock_Coverage_Weeks' in filtered_data.columns:
            ...
            week_4_movement = filtered_data['Week_4_Avg_Movement'].replace(0, np.nan)
            filtered_data['Weeks_of_Supply'] = filtered_data['Stock_Level'] / week_4_movement.fillna(1)
            
        # Calculate turnover rate
        filtered_data['Turnover_Rate'] = filtered_data['Week_4_Avg_Movement'] / filtered_data['Stock_Level'].replace(0, np.nan)
        filtered_data['Turnover_Rate'] = filtered_data['Turnover_Rate'].fillna(0)
        ...
        # Even more vertical spacing and adjusted heights for full-width visualization
        fig = make_subplots(
            rows=2, cols=1,
            shared_xaxes=True,
            vertical_spacing=0.5,  # Further increased spacing between subplots for full-width layout
            subplot_titles=("Stock Level & Inventory Projection", "Daily Demand Forecast"),
            row_heights=[0.6, 0.4]  # More balanced row heights for full-width visualization
        )
        ...
        if len(filtered_data) > 0:
            ...
            future_dates = pd.date_range(start=latest_date, periods=15)[1:]  # Skip today
            
            if 'Safety_Stock' in latest_data.columns:
                safety_stock = latest_data['Safety_Stock'].values[0]
                
                # Add safety stock for historical data
                fig.add_trace(
                    go.Scatter(
                        x=filtered_data['Date'],
                ...
                # Add safety stock for projected dates
                fig.add_trace(
                    go.Scatter(
                        x=future_dates,
                        y=[safety_stock] * len(future_dates),
                        mode='lines',
                        showlegend=False,
                        line=dict(color='red', dash='dot')
                    ),
            ...
            if 'Target_Stock' in latest_data.columns:
                ...
                # Add target stock for historical data
                fig.add_trace(
                    go.Scatter(
                        x=filtered_data['Date'],
                ...
                # Add target stock for projected dates
                fig.add_trace(
                    go.Scatter(
                        x=future_dates,
                        y=[target_stock] * len(future_dates),
                        mode='lines',
                        showlegend=False,
                        line=dict(color='green', dash='dot')
                    ),
        ...
        if apply_weather and location:
            try:
                ...
                # Add original demand to bottom subplot
                fig.add_trace(
                    go.Scatter(
                        x=filtered_data['Date'],
                        y=base_demand,
                        mode='lines',
                        name='Base Demand Forecast',
                        line=dict(color='purple', dash='dot')
                    ),
                    row=2, col=1
                )
                ...
                # Calculate and add future inventory projection based on adjusted demand
                latest_date = filtered_data['Date'].max()
                latest_idx = filtered_data[filtered_data['Date'] == latest_date].index
                
                if len(latest_idx) > 0:
                    # Get latest stock level
                    latest_stock = filtered_data.loc[latest_idx, 'Stock_Level'].values[0]
                    
                    # Project future dates (14 days)
                    future_dates = pd.date_range(start=latest_date, periods=15)[1:]  # Skip today
                    projected_stock = [latest_stock]
                    
                    # Simple projection: subtract daily demand from stock
                    for i in range(14):
                        # Use adjusted demand if available for this day, otherwise use avg
                        daily_demand = adjusted_demand[0] if i < len(adjusted_demand) else adjusted_demand.mean()
                        new_stock = max(0, projected_stock[-1] - daily_demand)
                        projected_stock.append(new_stock)
                    
                    # Remove first element which was the starting point
                    projected_stock = projected_stock[1:]
                    
                    # Add projection line to top subplot
                    fig.add_trace(
                        go.Scatter(
                            x=future_dates,
                            y=projected_stock,
                            ...
                            line=dict(color='rgba(0, 0, 255, 0.7)', width=2, dash='dot')
                        ),
                        row=1, col=1
                    )
                    
                    # Add shading for the projection
                    fig.add_trace(
                        go.Scatter(
                            x=future_dates,
                            y=[projected_stock[i] * 1.1 for i in range(len(projected_stock))],
                            mode='lines',
                            line=dict(width=0),
                            showlegend=False
                        ),
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=future_dates,
                            y=[projected_stock[i] * 0.9 for i in range(len(projected_stock))],
                            mode='lines',
                            fill='tonexty',
        ...
        fig.update_layout(
            title={
                'text': f"Inventory Projection with Demand Forecast for {product_name}",
            ...
            legend=dict(
                orientation="h", 
                yanchor="bottom", 
                y=1.12,  # Moved legend much higher
                xanchor="right", 
                x=1,
                font=dict(size=10),
                itemsizing='constant'
            ...
            height=1000,  # Further increased overall height for full-width visualizations
            margin=dict(t=220, b=120, l=100, r=100),  # Significantly increased margins for much better spacing
        ...
        fig.update_annotations(font_size=20, y=0.995, selector=dict(text="Stock Level"))
        fig.update_annotations(font_size=20, y=0.28, selector=dict(text="Daily Demand"))
    ...
    try:
        ...
        if current_stock < safety_stock:
            ...
            if projected_stock < safety_stock:
                ...
                # Consider demand forecast trend - if increasing, may need to order
                if forecast_available and len(daily_forecast) >= 7:
                    first_week = sum([demand for _, demand in daily_forecast[:7]])
                    second_week = sum([demand for _, demand in daily_forecast[7:14]]) if len(daily_forecast) >= 14 else first_week
                    
                    # If demand is trending up significantly, suggest an order
                    if second_week > first_week * 1.2:  # 20% increase
                        additional_need = int((second_week - first_week) * 0.5)  # Half of the increase
                        if projected_stock - additional_need < safety_stock:
                            order_quantity = max(0, int(target_stock - projected_stock + additional_need))
                        else:
                            order_quantity = 0
                    else:
                        order_quantity = 0
        ...
        # Determine inventory status based on weeks of supply and forecast demand
        if weeks_of_supply < 1:
            stock_status = 'Needs Reordering'
            status_color = 'danger'
        elif weeks_of_supply <= 3:
            # If forecast demand is high compared to historical average, adjust status
            if forecast_available and total_forecasted_demand > (week_4_avg_movement * 1.2):
                stock_status = 'Needs Reordering'
                status_color = 'danger'
            else:
                stock_status = 'Good'
        ...
        recommendation_card = dbc.Card(
            [
                ...
                dbc.CardBody([
                            ...
                            dbc.Card([
                                dbc.CardBody([
                                    dbc.Row([
                                        ...
                                        dbc.Col([
                                            html.H6(stock_status, className=f"text-{status_color} fw-bold"),
                                            html.Small(
                                                "(Based on forecast trend)" if forecast_available and total_forecasted_demand > (week_4_avg_movement * 1.2) else "",
                                    ...
                                    dbc.Row([
                                        ...
                                        dbc.Col([
                                            html.P(f"{int(current_stock)} units", className="fw-bold"),
                                            html.Small(
                                                "(Updated from stock count)" if adjust_clicks and stock_adjustment is not None else "",
                                                className="text-muted"
                                            )
                                        ], width=6)
                                    ]),
                    ...
                    dbc.Row([
                        ...
                        dbc.Col([
                            ...
                            dbc.Card([
                                dbc.CardBody([
                                    html.H5(
                                        f"Order {order_quantity} units now" if order_quantity > 0 else 
                                        "No reordering needed" if stock_status != 'Needs Reordering' else 
                                        "Reordering recommended based on forecast",
                                        className=f"text-center {'text-danger' if order_quantity > 0 or stock_status == 'Needs Reordering' else 'text-success'} fw-bold"
                                    ),
                                    html.Hr(),
                                    html.Div([
                                        html.P(
                                            "Based on recent stock count and demand forecast:" if stock_adjustment is not None else 
                                            "Based on current stock levels and demand forecast:",
                    ...
                    html.Div([
                        # Use stock status from data if available, with enhanced indicators
                        dbc.Badge(["Needs Reordering", 
                                  html.Small(" (After Stock Count)" if adjust_clicks and stock_adjustment is not None else "")], 
                                 color="danger", className="me-1") if stock_status == 'Needs Reordering' or weeks_of_supply < 1 else None,
...
def update_stock_velocity_chart(n_clicks, adjust_clicks, store, product, combined_data, go, np, stock_adjustment=None, adjustment_date=None):
    ...
    try:
        ...
        product_data['Turnover_Rate_4W'] = product_data['Week_4_Avg_Movement'] / product_data['On_Hand'].replace(0, np.nan)
        product_data['Turnover_Rate_4W'] = product_data['Turnover_Rate_4W'].fillna(0)
        product_data['Turnover_Rate_4W'] = product_data['Turnover_Rate_4W'].replace([np.inf, -np.inf], 0)
        
        # 13-week turnover rate for longer-term trend
        product_data['Turnover_Rate_13W'] = product_data['Week_13_Avg_Movement'] / product_data['On_Hand'].replace(0, np.nan)
        product_data['Turnover_Rate_13W'] = product_data['Turnover_Rate_13W'].fillna(0)
        product_data['Turnover_Rate_13W'] = product_data['Turnover_Rate_13W'].replace([np.inf, -np.inf], 0)
        ...
        if 'Sales' in product_data.columns:
            ...
            product_data['Stock_Velocity_28d'] = product_data['Rolling_Sales_28d'] / product_data['On_Hand'].replace(0, np.nan)
            product_data['Stock_Velocity_28d'] = product_data['Stock_Velocity_28d'].replace([np.inf, -np.inf], np.nan)
            product_data['Stock_Velocity_28d'] = product_data['Stock_Velocity_28d'].fillna(0)
        else:
            # Fall back to calculated turnover rates
            product_data['Stock_Velocity_28d'] = product_data['Turnover_Rate_4W']
            
        # Integrate forecast data if available to project future turnover rates
        if 'Daily_Sales_Forecast' in product_data.columns:
            latest_idx = product_data['Date'].idxmax()
            if latest_idx is not None:
                latest_date = product_data.loc[latest_idx, 'Date']
                forecast_data = product_data[product_data['Date'] >= latest_date].copy()
                
                if len(forecast_data) > 0:
                    # Calculate forecasted daily and weekly demand
                    avg_forecasted_daily = forecast_data['Daily_Sales_Forecast'].mean()
                    forecasted_weekly = avg_forecasted_daily * 7
                    
                    # Calculate projected turnover rate based on forecasted demand
                    latest_stock = product_data.loc[latest_idx, 'On_Hand']
                    if latest_stock > 0:
                        forecast_turnover_rate = forecasted_weekly / latest_stock
                        
                        # Add forecasted turnover rate to the latest data point
                        product_data.loc[latest_idx, 'Forecast_Turnover_Rate'] = forecast_turnover_rate
        ...
        # Add forecasted turnover rate if available
        if 'Forecast_Turnover_Rate' in product_data.columns:
            latest_idx = product_data['Date'].idxmax()
            if latest_idx is not None:
                latest_date = product_data.loc[latest_idx, 'Date']
                forecast_turnover = product_data.loc[latest_idx, 'Forecast_Turnover_Rate']
                
                # Add forecast point to show projected turnover rate
                fig.add_trace(
                    go.Scatter(
                        x=[latest_date],
                        y=[forecast_turnover],
                        mode='markers',
                        name='Forecasted Turnover Rate',
                        marker=dict(color='red', size=10, symbol='star'),
                        hovertemplate='Forecasted: %{y:.2f}x/week<extra></extra>'
                    ),
                    row=1, col=1
                )
                
                # Add projected trend line showing future turnover
                future_dates = pd.date_range(start=latest_date, periods=15)[1:]  # Skip today
                fig.add_trace(
                    go.Scatter(
                        x=future_dates,
                        y=[forecast_turnover] * len(future_dates),
                        mode='lines',
                        name='Projected Turnover',
        ...
        fig.update_layout(
            title={
                'text': f"Inventory Turnover Analysis for {product_name}<br><br><sup>Current: {current_turnover:.2f}x/week, Average: {avg_turnover:.2f}x/week{', Forecast: ' + str(product_data.loc[latest_idx, 'Forecast_Turnover_Rate']) + 'x/week' if 'Forecast_Turnover_Rate' in product_data.columns else ''}</sup><br><sup>{('Including forecast data' if 'Daily_Sales_Forecast' in product_data.columns else 'Based on historical data') + (' with recent stock count' if adjust_clicks and stock_adjustment is not None else '')}</sup>",
                'y': 0.99,  # Moved title even higher
                'x': 0.5,
                'xanchor': 'center',
                'yanchor': 'top',
                'font': {'size': 18}  # Further increased font size for better readability
            },
            legend=dict(
                orientation="h", 
                yanchor="bottom", 
            ...
            template="plotly_white",
            height=1000,  # Further increased height for full-width visualizations
            margin=dict(t=220, b=120, l=100, r=100),  # Significantly increased margins for much better spacing
            hovermode='closest'  # Better hover interaction
        )
        ...
        # Update subplot titles position for better spacing with full-width layout
        fig.update_annotations(font_size=20, y=0.995, selector=dict(text="Inventory Turnover Rates"))
        fig.update_annotations(font_size=20, y=0.28, selector=dict(text="On-Hand Inventory"))
...
def update_stock_penalty_chart(n_clicks, adjust_clicks, store, product, combined_data, go, np, stock_adjustment=None, adjustment_date=None):
    ...
    try:
        ...
        avg_cost = product_data['Cost'].mean() if 'Cost' in product_data.columns else 0
        ...
        # Only penalize for stock beyond 3 weeks of supply
        # Integrate demand forecast if available
        if 'Daily_Sales_Forecast' in product_data.columns:
            # Calculate forecast-based weeks of supply
            latest_idx = product_data['Date'].idxmax()
            if latest_idx is not None:
                latest_date = product_data.loc[latest_idx, 'Date']
                forecast_data = product_data[product_data['Date'] >= latest_date].copy()
                if len(forecast_data) > 0:
                    # Use forecasted demand for more accurate penalty calculation
                    avg_forecasted_daily = forecast_data['Daily_Sales_Forecast'].mean()
                    forecasted_weekly = avg_forecasted_daily * 7 if avg_forecasted_daily > 0 else product_data['Week_4_Avg_Movement'].mean()
                    
                    # Apply to all rows for trend visibility
                    product_data['Forecasted_Weekly_Demand'] = forecasted_weekly
        
        # Calculate excess stock penalty using forecast data if available
        product_data['Excess_Stock_Penalty'] = product_data.apply(
            lambda row: max(0, (row['Weeks_of_Supply'] - 3) * 
                         (row['Forecasted_Weekly_Demand'] if 'Forecasted_Weekly_Demand' in product_data.columns 
                          and row['Date'] >= latest_date else row['Week_4_Avg_Movement']) * 
                         avg_cost * weekly_carrying_cost)
            if row['Weeks_of_Supply'] > 3 else 0, axis=1
        )
        
        # Calculate lost sales cost (stockout penalty)
        margin = avg_price - avg_cost if avg_price > 0 and avg_cost > 0 else avg_price * 0.3  # Default to 30% margin
        
        # Use weeks of supply to calculate stockout penalty
        # Only penalize when weeks of supply is less than 1 week
        if 'Forecasted_Weekly_Demand' in product_data.columns:
            # Use the forecasted demand for stockout penalty calculation
            product_data['Stockout_Penalty'] = product_data.apply(
                lambda row: max(0, (1 - row['Weeks_of_Supply']) * 
                             max(row['Week_4_Avg_Movement'], row['Forecasted_Weekly_Demand']) * 
                             margin)
                if row['Weeks_of_Supply'] < 1 else 0, axis=1
            )
        else:
            # Fall back to historical demand
            product_data['Stockout_Penalty'] = product_data.apply(
                lambda row: max(0, (1 - row['Weeks_of_Supply']) * row['Week_4_Avg_Movement'] * margin)
                if row['Weeks_of_Supply'] < 1 else 0, axis=1
            )
        ...
        fig.update_layout(
            title={
                'text': f"Inventory Cost Impact for {product_name}<br><br><sup>Total Cost Impact: ${total_cost:.2f} (Excess: ${total_excess_cost:.2f}, Stockout Risk: ${total_stockout_cost:.2f})</sup><br><sup>{('Including forecast demand impact' if 'Forecasted_Weekly_Demand' in product_data.columns else 'Based on historical demand') + (' with recent stock count' if adjust_clicks and stock_adjustment is not None else '')}</sup>",
                'y': 0.99,  # Moved title even higher
                'x': 0.5,
                'xanchor': 'center',
                'yanchor': 'top',
        ...
        # Update subplot titles position for better spacing with full-width layout
        fig.update_annotations(font_size=20, y=0.995, selector=dict(text="Inventory Cost Impact"))
        fig.update_annotations(font_size=20, y=0.28, selector=dict(text="Weeks of Supply"))
...
def update_inventory_summary_stats(n_clicks, adjust_clicks, store, product, stock_adjustment, combined_data, html, dbc, app, adjustment_date=None):
    ...
    try:
        ...
        # Calculate summary statistics based on updated stock counts
        total_products = len(store_data)
        products_below_safety = len(store_data[(store_data['Stock_Status'] == 'Low') | (store_data['Stock_Status'] == 'Needs Reordering')])
        products_excess = len(store_data[store_data['Stock_Status'] == 'Excess']) + len(store_data[store_data['Stock_Status'] == 'Overstocked'])
        products_adequate = total_products - products_below_safety - products_excess
        
        # Add indicators for forecast-based adjustments
        forecast_influenced = 0
        if 'Daily_Sales_Forecast' in store_data.columns:
            # Count items where forecast is significantly higher than historical
            for idx, row in store_data.iterrows():
                if 'Avg_Weekly_Sales_4W' in row and 'Daily_Sales_Forecast' in row:
                    forecast_daily = row['Daily_Sales_Forecast']
                    historical_daily = row['Avg_Weekly_Sales_4W'] / 7 if row['Avg_Weekly_Sales_4W'] > 0 else 0
                    if forecast_daily > (historical_daily * 1.2):  # 20% higher
                        forecast_influenced += 1
                        ...
                        dbc.CardBody([
                            html.Div([
                                html.Div([f"{products_below_safety} Products Need Reordering ({pct_below:.1f}%)", 
                                          html.Small(" - Including recent stock count updates" if adjust_clicks and stock_adjustment is not None else "")], 
                                ...
                                html.Div([f"{products_excess} Products Overstocked ({pct_excess:.1f}%)",
                                          html.Small(" - Including forecast-influenced items" if forecast_influenced > 0 else "")], 
