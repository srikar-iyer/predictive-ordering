"""
Integrated forecasting module that connects Weighted ARIMA forecasting, inventory optimization, 
and pricing optimization for a complete predictive ordering system.
This module replaces the Random Forest models with Weighted Averaged ARIMA models.
"""
import pandas as pd
import numpy as np
import logging
...
from src.models.weighted_arima_model import WeightedARIMAModel, run_weighted_arima_forecasting
from src.optimization.profit_optimizer import ProfitOptimizer, run_profit_optimization

# Import settings
try:
    from config.settings import (
        COMBINED_DATA_FILE, WEIGHTED_ARIMA_FORECASTS_FILE, MODELS_DIR, STATIC_DIR,
    ...
    WEIGHTED_ARIMA_MODELS_DIR = os.path.join(MODELS_DIR, "weighted_arima")
...
logger = logging.getLogger('integrated_forecasting_weighted_arima')

class IntegratedForecasterWithWeightedARIMA:
    """
    Class that integrates Weighted ARIMA forecasting with pricing and inventory optimization
    """
    ...
    def generate_forecasts(self, days_to_forecast=30, use_existing=True, parallel=True):
        """
        Generate Weighted ARIMA forecasts
        ...
        """
        logger.info(f"Generating Weighted ARIMA forecasts for {days_to_forecast} days")
        ...
        try:
            ...
            self.forecasts = run_weighted_arima_forecasting(
    ...
    def optimize_inventory(self):
        ...
        try:
            ...
            for (store_id, item), group in self.forecasts.groupby(['Store_Id', 'Item']):
                ...
                forecast_7d = group[group['Days_In_Future'] <= 7]['Forecast'].sum()
                forecast_14d = group[group['Days_In_Future'] <= 14]['Forecast'].sum()
                forecast_30d = group[group['Days_In_Future'] <= 30]['Forecast'].sum()
                ...
                # Calculate weighted average daily sales (more weight on shorter-term forecasts)
                weighted_avg_daily_sales = (0.6 * avg_daily_sales_7d + 
                                          0.3 * avg_daily_sales_14d + 
                                          0.1 * avg_daily_sales_30d)
                
                # Calculate weeks of stock
                weeks_of_stock = current_stock / (weighted_avg_daily_sales * 7) if weighted_avg_daily_sales > 0 else 4
                
                # Calculate order point
                order_point = weighted_avg_daily_sales * 7 * MIN_STOCK_WEEKS
                
                # Calculate order quantity
                target_stock = weighted_avg_daily_sales * 7 * TARGET_STOCK_WEEKS
                ...
                if 'Lower_Bound' in group.columns and 'Upper_Bound' in group.columns:
                    ...
                    group['Interval_Width_Pct'] = (group['Upper_Bound'] - group['Lower_Bound']) / group['Forecast']
                    avg_confidence = 1.0 - min(1.0, group['Interval_Width_Pct'].mean())
                    confidence_level = avg_confidence * 100
                else:
                ...
                inventory_recommendations.append({
                    ...
                    'Avg_Daily_Sales': weighted_avg_daily_sales,
    ...
    def run_integrated_optimization(self, days_to_forecast=30, use_existing=True, 
                                  parallel=True, create_visuals=True):
        """
        ...
        """
        logger.info("Starting integrated optimization process with Weighted ARIMA models")
        ...
        try:
            ...
            self.generate_forecasts(days_to_forecast, use_existing, parallel)
    ...
    def create_visualizations(self):
        ...
        try:
            ...
            charts_dir = os.path.join(STATIC_DIR, 'images', 'integrated_weighted_arima')
            ...
            # Chart 5: Forecast Confidence Analysis
            if self.forecasts is not None:
                self._create_forecast_confidence_chart(charts_dir)
            
            # Chart 6: Integrated Dashboard
    ...
    def _create_forecast_comparison_chart(self, output_dir):
        ...
        agg_forecasts = self.forecasts.groupby('Date').agg({
            'Forecast': 'sum',
            'Lower_Bound': 'sum',
            'Upper_Bound': 'sum'
        }).reset_index()
    ...
    def _create_forecast_confidence_chart(self, output_dir):
        """
        Create forecast confidence chart
        ...
        # Calculate confidence interval as percentage of forecast
        forecasts_with_ci = self.forecasts.copy()
        forecasts_with_ci['CI_Width'] = forecasts_with_ci['Upper_Bound'] - forecasts_with_ci['Lower_Bound']
        forecasts_with_ci['CI_Pct'] = forecasts_with_ci['CI_Width'] / forecasts_with_ci['Forecast']
        
        # Group by days in future and calculate average confidence interval
        ci_by_day = forecasts_with_ci.groupby('Days_In_Future')['CI_Pct'].mean().reset_index()
        
        # Plot increasing uncertainty over time
        plt.plot(ci_by_day['Days_In_Future'], ci_by_day['CI_Pct'], 'b-', marker='o')
        
        # Add labels
        plt.xlabel('Days in Future')
        plt.ylabel('Confidence Interval (% of Forecast)')
        plt.title('Forecast Uncertainty Over Time')
        plt.grid(alpha=0.3)
        plt.tight_layout()
        
        # Save the chart
        plt.savefig(os.path.join(output_dir, 'forecast_confidence.png'), bbox_inches='tight')
        plt.close()
        
        # Create a second chart showing forecast confidence by product
        plt.figure(figsize=(12, 8))
        
        # Group by product and calculate average confidence interval
        ci_by_product = forecasts_with_ci.groupby('Product')['CI_Pct'].mean().sort_values(ascending=False).head(15)
        
        # Create bar chart
        ci_by_product.plot(kind='barh')
        
        # Add labels
        plt.xlabel('Confidence Interval (% of Forecast)')
        plt.ylabel('Product')
        plt.title('Forecast Uncertainty by Product')
        plt.grid(axis='x', alpha=0.3)
        plt.tight_layout()
        
        # Save the chart
        plt.savefig(os.path.join(output_dir, 'forecast_confidence_by_product.png'), bbox_inches='tight')
        plt.close()
    ...
    def _create_integrated_dashboard(self, output_dir):
        ...
        plt.plot(agg_forecasts['Date'], agg_forecasts['Forecast'], 'r-', label='Forecast')
        plt.fill_between(
            agg_forecasts['Date'],
            agg_forecasts['Lower_Bound'],
            agg_forecasts['Upper_Bound'],
        ...
        # Top middle: Price optimization summary
        plt.subplot(2, 3, 2)
        ...
        # Top right: Inventory status summary
        plt.subplot(2, 3, 3)
        ...
        # Bottom left: Forecast confidence over time
        plt.subplot(2, 3, 4)
        ...
        # Bottom middle: Top 5 products to order
        plt.subplot(2, 3, 5)
        
        top_orders = self.inventory_recommendations.sort_values('Order_Quantity', ascending=False).head(5)
        plt.bar(top_orders['Product'], top_orders['Order_Quantity'], color='skyblue')
        plt.title('Top 5 Products to Order')
        plt.xticks(rotation=45, ha='right')
        plt.grid(axis='y', alpha=0.3)
...
def main():
    ...
    """
    # Parse arguments
    parser = argparse.ArgumentParser(description='Integrated forecasting and optimization with Weighted ARIMA')
    ...
    parser.set_defaults(create_visuals=True, parallel=True)
    
    args = parser.parse_args()
    
    # Create integrated forecaster
    forecaster = IntegratedForecasterWithWeightedARIMA(data_path=args.data)
    ...
    logger.info("Integrated forecasting and optimization with Weighted ARIMA completed successfully")
